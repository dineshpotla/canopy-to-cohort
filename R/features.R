...
add_regeneration_gap_metrics <- function(data, established_quantile = 2 / 3, sensitivity_quantile = 0.75) {
  assert_columns(
    data,
    c("maple_ba_share", "maple_seedling_tpa", "maple_seedling_detected"),
    "analysis data"
  )

  outcome_eligible <- !is.na(data$maple_seedling_detected) & !is.na(data$maple_ba_share)
  ...
  established_share <- ifelse(outcome_eligible, data$maple_ba_share, NA_real_)
  ...
  data |>
    dplyr::mutate(
      ...
      potential_gap = dplyr::case_when(
        ...
        .data$maple_ba_share >= established_cut & .data$maple_seedling_detected == 0L ~ TRUE,
        TRUE ~ FALSE
      ),
      potential_gap_sensitivity = dplyr::case_when(
        is.na(.data$maple_seedling_detected) ~ NA,
        .data$maple_ba_share >= sensitivity_cut & .data$maple_seedling_detected == 0L ~ TRUE,
        TRUE ~ FALSE
      ),
