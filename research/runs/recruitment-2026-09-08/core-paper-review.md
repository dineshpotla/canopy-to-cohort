...
## September 10 revision

The subsequent [submission readiness review](submission-readiness-review.md)
updates the closest-work comparison, statistical limitations, computational
cross-checks, and manuscript disclosures. It preserves the core findings and
adds the remaining human approval, journal, and archive requirements. See the
latest artifact checksums and verification counts in `status.json`; the
September 9 full analysis run was not repeated during that revision.
